...
public class GestionFicherosImpl implements GestionFicheros {
	...
	public void elimina(String arg0) throws GestionFicherosException {
		...
		if(carpetaDeTrabajo.canWrite()){
			...
			if(file.delete()){
				System.out.println("Se ha eliminado "+arg0);
			}
			}else{
			throw new GestionFicherosException("Lo que estas intentando eliminar no existe");
		}
		
		}else{
			throw new GestionFicherosException("No tienes permiso para borrar esto");
		}
		actualiza();
	}
	...
	public String getInformacion(String arg0) throws GestionFicherosException {
		...
		SimpleDateFormat d = new SimpleDateFormat("MM/dd/yyy HH:mm:ss");
		
		strBuilder.append(d.format(file.lastModified()) + "\n");
		...
		if(file.isHidden()){
			...
			throw new GestionFicherosException("No esta oculto");
		}
		...
		if(file.isDirectory()){
			...
			strBuilder.append("Espacio ocupado: " + file.getUsableSpace() + "Bytes \n");
			strBuilder.append("Espacio total: " +file.getTotalSpace() + "Bytes \n");
		}
		
		//tamaño ficheros en bytes
		
		if(file.isFile()){
			strBuilder.append("El tamaño del fichero es: "+ file.length());
		}
	...
	public void renombra(String arg0, String arg1)
		...
		if(file.canWrite()){
		if(file.exists()){
			boolean bien = file.renameTo(file2);
			if(bien=true){
				System.out.println("Se ha cambiado el nombre con exito");
			}
		}
		}else{
			throw new GestionFicherosException("No tienes permiso para cambiar el nombre");
		}
		actualiza();
	}
